...
def main():
    ...
    root = args.folder if args.folder else '../test/test_seg'
    ...
    batch_size = 2048
    data = np.zeros((batch_size, max_point_num, 3))  # 初始化大小为batch_size*点云大小307200*3的数据（一次抓取2048张图像？）
            ...
            for category in sorted(os.listdir(data_folder)):    # data文件夹下的“01”,“02”等分类文件夹
                ...
                for filename in sorted(os.listdir(data_category_folder)):   # data下分类文件夹内每一个文件名
                    ...
                    coordinates = [[float(value) for value in xyz.split(' ')]
                                   for xyz in open(data_filepath, 'r') if len(xyz.split(' ')) == 3] # TODO:读出文件中数据，
